...
FEATURE_PATH = os.path.join(os.path.dirname(__file__), "../style_rewrites/get_style_rewrite.feature")
scenarios(FEATURE_PATH)


@given("a style rewrite has been created", target_fixture="workflow_response")
def create_style_rewrite(client: MarkupAI, sample_file: BufferedReader) -> WorkflowResponse:
...
def get_style_rewrite_without_polling(client: MarkupAI, workflow_response: WorkflowResponse) -> RewriteResponse:
    return client.style_rewrites.get_style_rewrite(workflow_id=workflow_response.workflow_id)
...
def get_style_rewrite_with_polling(client: MarkupAI, workflow_response: WorkflowResponse) -> RewriteResponse:
    return get_workflow_with_polling(
        get_workflow_fn=client.style_rewrites.get_style_rewrite, workflow_id=workflow_response.workflow_id
    )
...
def check_style_rewrite_workflow_info(get_style_rewrite_response: RewriteResponse):
    ...
    # TODO: More detailed checks can be added here for config, original content, and rewrite
